...
async function fetchJWKS() {
  try {
    ...
    throw new Error(`Unable to fetch JWKS from auth service at ${AUTH_SERVICE}.`);
  }
}
...
async function fetchStudents(req) {
  ...
  try {
    console.log(`Attempting to fetch students from: ${STUDENT_SERVICE}`);
    ...
    console.error(`Error fetching students from ${STUDENT_SERVICE}:`, errorData);
...
async function fetchCourses(req) {
  ...
  try {
    console.log(`Attempting to fetch courses from: ${COURSE_SERVICE}`);
    ...
    const errorData = error.response ? error.response.data : error.message;
    console.error(`Error fetching courses from ${COURSE_SERVICE}:`, errorData);
